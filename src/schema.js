...
const Device = new GraphQLObjectType({
  ...
  fields: {
    ...
    name: {
      ...
      resolve ({ uuid }, args, { loaders }, info) {
        return loaders.device.load(uuid).then(device => device.name)
      }
    },
    mac: {
      type: GraphQLString,
      ...
      resolve ({ uuid }, args, { loaders }, info) {
        return loaders.device.load(uuid).then(device => device.mac)
      }
    },
    userID: {
      type: GraphQLInt,
      ...
      resolve ({ uuid }, args, { loaders }, info) {
        return loaders.device.load(uuid).then(device => device.userId)
      }
    },
    sensors: {
      type: Sensors,
      ...
      resolve ({ uuid }, { period, averageBy }, { loaders }, info) {
